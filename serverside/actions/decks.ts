...
export async function processBulkDeckAction(
  ...
  try {
    ...
    // This allows PostgreSQL to handle the bulk insertion and potential conflicts atomcially
    const { error: bulkError } = await supabase.rpc('handle_bulk_import', {
      items: deckItems as any,
      admin_id: user.id, // Re-using parameter for ownership/audit
      // In a real deck specific RPC, we'd pass deck_id
    });

    // NOTE: In a true production deck-import, you'd want a specific RPC like 'import_deck_items'
    // but here we use the handle_bulk_import as an exemplar for batch operations.

    if (bulkError) {
        // Rollback strategy: In a real SQL transaction within RPC this is automatic.
        // If done via client, it's not. That's why RPC is preferred.
